...
- `auth.js` — there's no user table; admin auth is a single shared `ADMIN_PASSWORD` checked in the login route, which then issues a JWT (`role: "admin"`, 30m expiry). `requireAdmin` middleware gates the admin-only routes (booking list/status/fare, push-token registration).
- `validation.js` — zod schemas: `bookingSchema` (includes a honeypot `website` field for spam bots, and normalizes legacy `tripType` values), `statusSchema`, `fareSchema`, `pushTokenSchema`.
...
**Admin flow**: `POST /api/auth/login` → JWT (shared by both the CRM-side web usage and the mobile app — there's one admin identity, not per-platform accounts). `GET /api/bookings`, `PATCH /api/bookings/:id/status`, `PATCH /api/bookings/:id/fare` (saves the price confirmed with the customer, into `confirmedFare`), and `POST /api/admin/push-tokens` all require `requireAdmin`. Both the status and fare routes reject with `409 BOOKING_ALREADY_COMPLETED` once a booking's status is `completed` — nothing about a completed booking can change after the fact.
