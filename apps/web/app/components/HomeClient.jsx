...
export default function HomeClient() {
  ...
  function updateBooking(field, value) {
    if (!touchedFieldsRef.current.has(field)) {
      if (touchedFieldsRef.current.size === 0) {
        trackClarityEvent("booking_form_started");
      }
      touchedFieldsRef.current.add(field);
      if (field === "date" || field === "time" || field === "email") {
        trackClarityEvent(`booking_field_touched_${field}`);
      }
    }
  ...
  async function submitBooking(event) {
    ...
    if (Object.keys(errors).length > 0) {
      ...
      Object.keys(errors).forEach((field) => trackClarityEvent(`booking_validation_error_${field}`));
      return;
    }
    setFieldErrors({});
    ...
    try {
      ...
      trackClarityEvent("booking_submit_error");
    }
  }
